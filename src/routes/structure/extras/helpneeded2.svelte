<div id="content-box-main1">
    <div style="display: flex;">
        <div>
            <img src="/images/character-art/Pompom/Pom-Pom_New_Sticker_03.png" alt="pompom" style="height: 100px;">
        </div>
        <div>
            <h1 style="margin-top: 7.5px; line-height: 10px; font-size: 30px;">&nbsp;This page is being worked on and or needs more information!</h1><br>
            <p style="line-height: 20px; margin-top: -7.5px;">&nbsp;&nbsp;Sorry for the inconvenience but if you would like to help add information, please check out the GitHub <br>&nbsp; page and submit a new issue!</p>
            <p style="line-height: 20px; margin-top: -7.5px;">&nbsp;&nbsp;<a id="button" href="https://github.com/KittenzExe/hsrtools.kittenzexe" target="_blank" rel="noopener noreferrer">GitHub Page</a> <a id="button" href="https://github.com/KittenzExe/hsrtools.kittenzexe/blob/main/CONTRIBUTING.md" target="_blank" rel="noopener noreferrer">Contribution Docs</a> <a id="button" href="https://github.com/KittenzExe/hsrtools.kittenzexe/issues" target="_blank" rel="noopener noreferrer">Issue Report (or Feature Report)</a> <a id="button" href="../development/devtree">DevTree</a> <br> &nbsp;&nbsp;(Your help will be much appreciated!)</p>
        </div>
    </div>
</div>
<div id="spacer"></div>

<style>
  @font-face { font-family: DIN; src: url('/fonts/DIN Bold.ttf'); } 

  #content-box-main1 {
      width: 64%;
      height: 150px;
      margin-left: 500px;
      padding: 20px;
      top: 20px;
      left: -10px;
      bottom: 10px;
      position: relative;
      overflow: hidden;
      background-color: #43454e;
      border-width: 5px;
      border-style: solid;
      border-color: #b9141c;
      border-top-right-radius: 50px;
      border-bottom-left-radius: 50px;
      font-family: DIN;
      font-size: 20px;
      line-height: 5px;
      color: #fef6ed;
  }

  #button {
      background-color: #b9141c;
      border: 0;
      border-radius: 10px;
      color: #fef6ed;
      padding: 5px 10px;
      text-align: center;
      text-decoration: none;
      font-size: 16px;
      margin: 4px 2px;
      font-family: DIN;
      font-size: 20px;
      line-height: 40px;
      cursor: pointer;
    }

  #spacer {
    height: 30px;
  }

  @media screen and (max-width: 1050px) {
      #content-box-main1 {
        margin-left: 25px;
      }
  }
</style>